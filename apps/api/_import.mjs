...
const res = await fetch(`${BASE}/projects/${NID}/import/website/stream`, {
  method: 'POST',
  headers: { 'content-type': 'application/json', cookie, accept: 'text/event-stream' },
  body: JSON.stringify({ url: 'https://www.burmeister.com.na/', maxPages: 40, maxDepth: 3 }),
});
...
while (true) {
  ...
  for (const p of parts) {
    ...
    try {
      ...
      const tag = ev.phase || ev.type || ev.event || '';
      const detail = ev.detail || ev.message || (ev.url ? ev.url.slice(-50) : '') || '';
      const msg = `${tag} ${detail}`.trim();
      if (msg && msg !== last) { console.log('  •', msg); last = msg; }
      if (ev.report || ev.result || tag === 'done' || tag === 'complete') {
        writeFileSync(SP + '/import-report.json', JSON.stringify(ev, null, 2));
        console.log('REPORT SAVED');
      }
      if (ev.error || tag === 'error') console.log('  !! ERROR', JSON.stringify(ev).slice(0, 300));
