...
// Foundation import, HOMEPAGE ONLY (maxPages:1, maxDepth:0).
const res = await fetch(`${BASE}/projects/${NID}/import/website/stream?foundation=1`, {
  ...
  body: JSON.stringify({ url: 'https://www.rbs.com.na/', maxPages: 1, maxDepth: 0 }),
});
...
while (true) {
  ...
  for (const p of parts) {
    ...
    try {
      ...
      const detail = ev.detail || ev.message || (ev.url ? ev.url.slice(-60) : '') || '';
