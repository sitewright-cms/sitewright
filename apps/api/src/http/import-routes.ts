...
// (flat) or the legacy `/media/<slug>/<id>/<file>.js` (optional middle segment), optionally
// `defer`/`async` — the importer's JS-hosting output. No inline body, no foreign src, no `on*`/other
// attributes. ONLY the `scripts` slot may carry these; everything else stays script-free.
