...
// Bound the total GENERATED bytes a single build writes to disk (pages, CSS, sitemap, search index —
// copied media assets are not counted here). A pathological project could otherwise fill the disk
// during the in-process build.
//
// ★ 100 MiB was too low for an ordinary site, and the comment claimed it was "operator-configurable"
// while nothing wired it to anything. A 1,162-page bilingual school site generated ~102 MiB of HTML —
// so EVERY build failed, the preview silently kept serving the last successful one, and the site
// looked like it was missing content that was in fact present. 512 MiB matches the import worker's
// ceiling and leaves room for a large multilingual site; SW_MAX_OUTPUT_MB overrides it.
const DEFAULT_MAX_OUTPUT_BYTES = 512 * 1024 * 1024;

/** Operator override, in MiB (`SW_MAX_OUTPUT_MB=1024`). Invalid/absent → the default above. */
export function configuredMaxOutputBytes(env: NodeJS.ProcessEnv = process.env): number {
  const raw = Number(env.SW_MAX_OUTPUT_MB);
  return Number.isFinite(raw) && raw > 0 ? Math.floor(raw) * 1024 * 1024 : DEFAULT_MAX_OUTPUT_BYTES;
}
...
export async function buildSite(opts: BuildSiteOptions): Promise<ReleaseManifest> {
  ...
  const maxOutputBytes = opts.maxOutputBytes ?? configuredMaxOutputBytes();
    ...
    const pageFailures: PageBuildFailure[] = [];
    {
      ...
      for (const route of routes) {
        ...
        if (bytes > maxOutputBytes) {
          throw new PublishError(
            `published site exceeds the maximum output size — generated ${(bytes / 1024 / 1024).toFixed(0)} MB of ${(maxOutputBytes / 1024 / 1024).toFixed(0)} MB across ${routes.length} routes (raise SW_MAX_OUTPUT_MB, or reduce the number of pages)`,
          );
        }
      }
    }
