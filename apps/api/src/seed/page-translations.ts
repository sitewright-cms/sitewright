...
export const PAGE_TRANSLATIONS: Record<string, Record<string, string>> = {
  ...
  // comp_cookie (migrated from page.data)
  'comp_cookie.cc_intro': { de: 'Ein kleines Einwilligungsbanner, das die Runtime nur einblendet, bis der Besucher zustimmt — die Wahl wird in localStorage gemerkt, also erscheint es einmal und nie wieder. Es lebt in einem Skeleton-Slot und ist damit auf jeder Seite vorhanden; das echte hast du beim ersten Besuch am unteren Rand gesehen.', es: 'Un pequeño banner de consentimiento que el runtime muestra solo hasta que el visitante acepta — la elección se recuerda en localStorage, así que aparece una vez y nunca más. Vive en un slot de plantilla y por eso está en todas las páginas; el real lo viste al pie de tu primera visita.' },
  'comp_cookie.sec_preview_t': { de: 'So sieht es aus', es: 'Qué aspecto tiene' },
  'comp_cookie.sec_preview_d': { de: 'Eine statische Vorschau des Banners (hier gezeigt, damit es auch sichtbar ist, nachdem du das echte akzeptiert hast). Das Live-Banner ist am unteren Rand fixiert und gleitet beim ersten Besuch herein.', es: 'Una vista previa estática del banner (mostrada aquí para que sea visible incluso después de aceptar el real). El banner en vivo queda fijo al pie del viewport y se desliza en la primera visita.' },
  'comp_cookie.cc_text': { de: 'Wir verwenden wenige notwendige Cookies für den Betrieb dieser Website und anonyme Statistiken zu ihrer Verbesserung.', es: 'Usamos unas pocas cookies esenciales para que el sitio funcione y estadísticas anónimas para mejorarlo.' },
  'comp_cookie.cc_more': { de: 'Mehr erfahren', es: 'Más información' },
  'comp_cookie.cc_accept': { de: 'Alles klar', es: 'Entendido' },
  'comp_cookie.sec_how_t': { de: 'So funktioniert’s', es: 'Cómo funciona' },
  'comp_cookie.sec_how_d': { de: 'Einmal in einem Skeleton-Slot anlegen (im Footer oder einem eigenen Slot). Der Server rendert es mit einem hidden-Attribut; die Runtime prüft localStorage und blendet es nur ein, wenn keine Wahl gespeichert ist, und blendet es nach Druck auf die Zustimmen-Schaltfläche dauerhaft aus. Das Verhalten trägt der Marker, nicht das ausgezeichnete HTML. Die Zustimmung wird standardmäßig unter dem Schlüssel sw-cookie-consent gespeichert — mit einem optionalen data-cookiename legst du einen eigenen Schlüssel fest, sodass zwei unabhängige Banner ihre Zustimmung getrennt verwalten.', es: 'Créalo una vez en un slot de plantilla (el pie u otro propio). El servidor lo renderiza con un atributo hidden; el runtime consulta localStorage y lo muestra solo cuando no hay elección guardada, y lo oculta de forma permanente al pulsar el botón de aceptar. El comportamiento lo aporta el marcador, no el HTML escrito. El consentimiento se guarda bajo la clave sw-cookie-consent por defecto — añade un data-cookiename opcional para usar tu propia clave, de modo que dos banners independientes registren el consentimiento por separado.' },
  'comp_cookie.sec_nojs_t': { de: 'Ohne JavaScript', es: 'Sin JavaScript' },
  'comp_cookie.sec_nojs_d': { de: 'Es erscheint gar kein Banner — und ohne Skripte gibt es nichts zu setzen oder zu speichern, die Seite wird einfach unverändert ausgeliefert.', es: 'No aparece ningún banner — y sin scripts no hay nada que establecer ni guardar, así que la página se sirve tal cual.' },
