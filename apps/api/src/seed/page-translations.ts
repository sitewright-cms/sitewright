...
export const PAGE_TRANSLATIONS: Record<string, Record<string, string>> = {
  ...
  'comp_banner.bn_intro': { de: 'Ein frei gestaltbares Banner, das du überall platzierst — die Runtime blendet es ein und merkt sich die Schließung in localStorage, damit es nicht nervt. Du schreibst den Inhalt und die Aktions-Schaltflächen; Häufigkeit, Position und Schlummern sind Attribut-Schalter. Es ist NICHT das Cookie-Banner — das ist der automatisch eingefügte Consent Manager.', es: 'Un banner de contenido libre que colocas donde quieras — el runtime lo muestra y recuerda el cierre en localStorage para que no moleste. Tú escribes el cuerpo y los botones de acción; la frecuencia, la posición y el aplazamiento son interruptores de atributo. NO es el banner de cookies — ese es el Consent Manager autoinyectado.' },
  'comp_banner.sec_preview_t': { de: 'So sieht es aus', es: 'Qué aspecto tiene' },
  'comp_banner.sec_preview_d': { de: 'Eine statische Vorschau einer Promo-Karte in der Ecke (hier gezeigt, damit sie ohne Schließen eines echten Banners sichtbar ist). Das echte Banner ist in der gewählten Ecke fixiert und blendet sich beim Einblenden ein.', es: 'Una vista previa estática de una tarjeta promocional en la esquina (mostrada aquí para que sea visible sin descartar una real). El banner real queda fijo en la esquina elegida y se desvanece al aparecer.' },
  'comp_banner.bn_text': { de: 'Wirf einen Blick in den Shop, um unser neuestes Produkt zu sehen.', es: 'Echa un vistazo a la tienda para ver nuestro último producto.' },
  'comp_banner.bn_dismiss': { de: 'Nicht mehr anzeigen', es: 'No volver a mostrar' },
  'comp_banner.sec_how_t': { de: 'So funktioniert’s', es: 'Cómo funciona' },
  'comp_banner.sec_how_d': { de: 'Lege es einmal in einem Slot oder Seitenkörper mit dem hidden-Attribut an. Die Runtime blendet es ein, und die Schließen-Schaltflächen merken sich die Wahl: data-sw-part="dismiss" folgt data-frequency (once / session / days:N / always), "dismiss-forever" blendet es dauerhaft aus, und "remind" schlummert für data-remind-days. Gib jedem Banner eine EINDEUTIGE data-sw-banner-id, damit Schließungen getrennt verfolgt werden. data-position platziert es (Ecken / oben / unten / mittig / inline) und data-delay blendet es nach N ms oder beim ersten Scrollen ein.', es: 'Créalo una vez en un slot o cuerpo de página con el atributo hidden. El runtime lo muestra y los botones de cierre recuerdan la elección: data-sw-part="dismiss" sigue a data-frequency (once / session / days:N / always), "dismiss-forever" lo oculta de forma permanente y "remind" lo aplaza durante data-remind-days. Da a cada banner un data-sw-banner-id ÚNICO para que los cierres se registren por separado. data-position lo coloca (esquinas / arriba / abajo / centro / inline) y data-delay lo muestra tras N ms o el primer desplazamiento.' },
