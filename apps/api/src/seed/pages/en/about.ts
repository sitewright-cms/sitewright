...
<section class="bg-base-200/60">
  <div class="mx-auto max-w-6xl px-6 py-24">
    ...
    <div class="mt-12 grid gap-6 md:grid-cols-3">
      <div class="nw-card rounded-3xl bg-base-100 p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5" data-sw-animation="fade-up"><span class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/15 to-secondary/15 text-primary">${icon('diamond', 'h-5 w-5')}</span><h3 class="mt-5 text-lg font-bold tracking-tight" data-sw-translate="about.v1_t">Craft over churn</h3><p class="mt-2 leading-relaxed text-base-content/60" data-sw-translate="about.v1_b">We sweat the details most teams skip — because details are what people feel.</p></div>
      <div class="nw-card rounded-3xl bg-base-100 p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5" data-sw-animation="fade-up" data-sw-delay="100"><span class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/15 to-secondary/15 text-primary">${icon('lightning', 'h-5 w-5')}</span><h3 class="mt-5 text-lg font-bold tracking-tight" data-sw-translate="about.v2_t">Speed is a feature</h3><p class="mt-2 leading-relaxed text-base-content/60" data-sw-translate="about.v2_b">Every site we ship is static, optimized, and built to load instantly.</p></div>
      <div class="nw-card rounded-3xl bg-base-100 p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5" data-sw-animation="fade-up" data-sw-delay="200"><span class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/15 to-secondary/15 text-primary">${icon('handshake', 'h-5 w-5')}</span><h3 class="mt-5 text-lg font-bold tracking-tight" data-sw-translate="about.v3_t">Plain dealing</h3><p class="mt-2 leading-relaxed text-base-content/60" data-sw-translate="about.v3_b">Fixed scopes, clear timelines, and honest advice — even when it costs us the upsell.</p></div>
    </div>
  </div>
</section>
...
<section class="mx-auto max-w-6xl px-6 pb-28">
  ...
  <div class="mt-10" data-sw-component="lightbox" data-sw-block="Lightbox" aria-label="{{sw-translate "about.aria_gallery"}}">
    <div data-sw-part="grid" class="gap-4 !grid-cols-2 md:!grid-cols-4">
      ...
      {{else}}
      <p class="col-span-full text-base-content/50" data-sw-translate="about.gal_empty">No photos yet — drop some into the Studio folder.</p>
      {{/sw-folder}}
    </div>
  </div>
</section>`,
  ...
  <p class="mt-5 text-lg leading-relaxed text-base-content/60" data-sw-translate="careers.ca_intro">A small team means your work ships, your name is on it, and nobody manages the manager. These roles are open right now.</p>
  ...
  <div class="mt-14 space-y-7">
    ...
    {{else}}
    <p class="rounded-3xl border border-dashed border-base-300 p-10 text-center text-base-content/50" data-sw-translate="careers.ca_empty">No openings right now — but we always read a great portfolio.</p>
    {{/each}}
  </div>
