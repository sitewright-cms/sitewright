  ...
  <div class="grid items-start gap-12 lg:grid-cols-2 lg:gap-20">
    <div class="nw-rise">
      ...
      <ul class="mt-10 list-none space-y-4 p-0">
        <li class="flex items-center gap-4"><span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/15 to-secondary/15 text-primary">${icon('envelope', 'h-5 w-5')}</span><a class="nw-underline font-semibold text-primary no-underline" href="mailto:hello@northwindstudio.com">{{company.email}}</a></li>
        ...
        <li class="flex items-center gap-4"><span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/15 to-secondary/15 text-primary">${icon('clock', 'h-5 w-5')}</span><span class="font-medium text-base-content/80" data-sw-translate="contact.c_hours">Mon–Fri, 9–6 PT</span></li>
      </ul>
      ...
      <div class="mt-12" data-sw-component="modal" data-sw-block="Modal" data-close-label="{{sw-translate "contact.c_close"}}">
        <button type="button" data-sw-part="open" class="btn btn-outline">${icon('calendar', 'h-5 w-5')} <span data-sw-translate="contact.c_modal_btn">What happens on the intro call?</span></button>
