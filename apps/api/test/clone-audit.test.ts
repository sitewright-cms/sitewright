...
describe('behaviouralChecks', () => {
  ...
  describe('header clearance', () => {
    ...
    it('FAILS a token that under-declares, and says which viewport and by how much', () => {
      ...
      expect(c.advisory).toBeFalsy(); // gates: the audit render now agrees with published + preview
  ...
  it('a check with NOTHING to check is N/A, not a pass — it must not pad the score', () => {
    ...
    // 4, not 3: the header-height check is n/a too when there is no fixed header to measure — the same
    // discipline, so it must not pad the score either. (The advisory spacer check is n/a as well, but
    // advisories are outside this count entirely.)
