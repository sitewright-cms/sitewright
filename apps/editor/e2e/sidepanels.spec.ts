import { test, expect } from '@playwright/test';
import { signUp } from './helpers.js';

const stamp = Date.now();

// The reusable edge side panels (Library left, Assets right): a small brand tab on each edge that
// CLICKS open a fixed-size drawer which slides in over a dim backdrop. The drawer STAYS open until
// dismissed — click the backdrop, press Escape, or hit the header ×. One drawer at a time (an open
// drawer's backdrop covers the other tabs). Always present for owners.
test('side panels: Library + Assets tabs click open as drawers; backdrop + Escape close them', async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 800 });
  await signUp(page, `panels-${stamp}@e2e.test`);
  await page.getByRole('button', { name: 'New project' }).click();
  await page.getByLabel('Project name').fill('Panels Site');
  await page.getByLabel('Project slug').fill(`panels-${stamp}`);
  await page.getByRole('button', { name: 'Create project' }).click();

  // Both collapsed edge tabs are present.
  const libTab = page.getByRole('button', { name: 'Open System Library' });
  const assetsTab = page.getByRole('button', { name: 'Open File Manager' });
  await expect(libTab).toBeVisible();
  await expect(assetsTab).toBeVisible();

  // The Library panel is collapsed (aria-hidden) until opened. (Query by attribute — a hidden
  // region is excluded from the accessibility tree, so getByRole wouldn't see it.)
  const library = page.locator('[role="region"][aria-label="System Library"]');
  await expect(library).toHaveAttribute('aria-hidden', 'true');

  // Click the tab → the drawer slides open and its sections show, and it STAYS open as the pointer
  // moves away (no hover dependency).
  await libTab.click();
  await expect(library).toHaveAttribute('aria-hidden', 'false');
  await expect(library.getByText('Google Fonts', { exact: true })).toBeVisible();
  await page.mouse.move(640, 400);
  await expect(library).toHaveAttribute('aria-hidden', 'false');

  // Escape closes the drawer.
  await page.keyboard.press('Escape');
  await expect(library).toHaveAttribute('aria-hidden', 'true');

  // The Assets tab (right edge) shares the same drawer and opens its file browser on click.
  await assetsTab.click();
  const assets = page.locator('[role="region"][aria-label="File Manager"]');
  await expect(assets).toHaveAttribute('aria-hidden', 'false');
  await expect(assets.getByLabel('Upload files')).toBeVisible();

  // Clicking the dim backdrop (away from the right-edge panel) closes it.
  await page.mouse.click(200, 400);
  await expect(assets).toHaveAttribute('aria-hidden', 'true');

  // Dragging OS files onto the COLLAPSED File Manager tab opens the drawer, so drag-and-drop upload
  // works without first opening it (the file browser's drop zone is unreachable while collapsed). A
  // DataTransfer holding a File reports `types` containing 'Files' — exactly what the tab keys on.
  const fileDrag = await page.evaluateHandle(() => {
    const dt = new DataTransfer();
    dt.items.add(new File(['x'], 'photo.png', { type: 'image/png' }));
    return dt;
  });
  await assetsTab.dispatchEvent('dragenter', { dataTransfer: fileDrag });
  await expect(assets).toHaveAttribute('aria-hidden', 'false');
  await expect(assets.getByLabel('Upload files')).toBeVisible();
});
