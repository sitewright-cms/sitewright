  ...
  return createPortal(
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-2.5" style={{ height: HEADER_H }}>
        ...
        <button type="button" aria-label="Close preview" onClick={onClose} className="text-slate-500 dark:text-slate-400 transition hover:text-slate-700 dark:hover:text-slate-200">
          <X className="h-4 w-4" />
        </button>
      </div>
      <div className="relative" style={{ height: CARD_H - HEADER_H }}>
        {!loaded && <div className="absolute inset-0 grid place-items-center text-xs text-slate-500 dark:text-slate-400">Rendering…</div>}
