...
const DAISYUI_ITEMS: LibraryItem[] = [
  ...
  daisy('Filter', 'filter radio chips reset segmented facets', 'A radio-button filter group with a reset (CSS-only via form reset).', '<form class="filter">\n  <input class="btn btn-square" type="reset" value="×" />\n  <input class="btn" type="radio" name="f" aria-label="Design" />\n  <input class="btn" type="radio" name="f" aria-label="Web" />\n  <input class="btn" type="radio" name="f" aria-label="Branding" />\n</form>'),
  // --- Layout ---
  daisy('Divider', 'divider separator rule', 'A labelled divider (vertical/horizontal).', '<div class="divider">OR</div>'),
  daisy('Drawer', 'drawer sidebar offcanvas', 'Sidebar layout (CSS-only toggle).', '<div class="drawer">\n  <input id="sw-drawer-demo" type="checkbox" class="drawer-toggle" />\n  <div class="drawer-content"><label for="sw-drawer-demo" class="btn btn-primary drawer-button">Open</label></div>\n  <div class="drawer-side"><ul class="menu bg-base-200 min-h-full w-64 p-4"><li><a href="/">Home</a></li></ul></div>\n</div>'),
...
export const PARALLAX_ITEMS: LibraryItem[] = [
  {
    ...
    keywords: 'parallax scroll move translate slide depth data-sw-parallax-translate',
    description: 'Slide an element between two pixel offsets as it crosses the viewport. Add data-sw-parallax-axis="x" for horizontal.',
    example: '<div data-sw-parallax-translate="40,-40">…</div>',
  },
  {
    id: 'px-opacity',
    name: 'Fade (opacity)',
    keywords: 'parallax scroll fade opacity appear data-sw-parallax-opacity',
    description: 'Interpolate opacity from,to across the element’s window of the viewport pass-through.',
  ...
  },
  {
    ...
    description: 'Interpolate scale from,to; composes with translate on one transform.',
  ...
  },
  {
    ...
    description: 'Interpolate a blur radius (px) from,to. Heavier to composite — use sparingly.',
    example: '<div data-sw-parallax-blur="8,0">…</div>',
  },
  {
    id: 'px-range',
    name: 'Anchor window (-range)',
    keywords: 'parallax scroll range window anchor centre reveal trigger data-sw-parallax-range',
    description: 'Anchor an effect to a window of the pass-through (0 enter · 0.5 centre · 1 leave). "0,0.5" finishes while centred; set data-sw-parallax-range on the element for a default.',
    example: '<div data-sw-parallax-opacity="0,1" data-sw-parallax-opacity-range="0,0.5">…</div>',
  },
  {
    id: 'px-inout',
    name: 'In → hold → out (-out)',
    keywords: 'parallax scroll out in reverse exit appear disappear data-sw-parallax-out',
    description: 'A window shorter than the full pass-through leaves room for an OUT phase: animate in, hold, then back out (optional -out-range).',
    example: '<div data-sw-parallax-opacity="0,1" data-sw-parallax-opacity-range="0,0.5" data-sw-parallax-opacity-out="1,0">…</div>',
  },
  {
    id: 'px-scene',
    name: 'Depth scene (stacked layers)',
    keywords: 'parallax scroll background hero section scene layer depth stack data-sw-parallax-scene',
    description: 'A clipping container of stacked, absolutely-positioned layers — the background is just the bottom layer, each moving independently (no-JS → a static clipped scene).',
    example:
      '<section data-sw-parallax-scene>\n  <div data-sw-parallax-layer data-sw-parallax-translate="70,-70" style="inset:-14% 0;background-image:url(/media/<id>/<file>);background-size:cover"></div>\n  <div data-sw-parallax-layer>…your content…</div>\n</section>',
  },
];
...
export const LIBRARY_SECTIONS: LibrarySection[] = [
  ...
  { category: 'flags', label: 'Country flags', blurb: 'Full-color country flags — the EU included. Pick a shape with the row above; insert with {{sw-icon "flag:de"}} (rectangular) or {{sw-icon "flag:de-circle"}} (round).', items: [], lazy: 'flags' },
