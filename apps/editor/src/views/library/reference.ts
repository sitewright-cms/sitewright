  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        description:
          'Emits the value as an inert data island — <script type="application/json" id="…">…</script> — for page JavaScript to read with JSON.parse(document.getElementById(\'…\').textContent). This is the ONLY way to get structured data to a script: a template may not interpolate inside a <script> body at all (that is a template error, because a value there could close the tag), so the helper emits the whole element and escapes the payload itself. Use it for a list a script filters, sorts or paginates in the browser, for a widget\'s configuration, or with type="application/ld+json" for your own structured data. Add size="sm" to size the images it carries (xs|sm|md|lg|xl): fields= can only pick a field, never transform it, so without it an island\'s image URLs are published at the default variant and a script painting small cards downloads the largest one. For a list too large to inline, declare a data FILE instead (website.dataFiles) and fetch it from data/&lt;name&gt;.json: an island is re-sent with every page view, a file ships once and is cached. A folder-sourced file can carry TWO urls per image — `size` for the tile and `full` for what a lightbox opens — because one variant cannot be both a 350px thumbnail and a full-screen photo.',
        args: [
          ...
          { name: 'fields', desc: 'NAMED, optional: comma-separated field names kept from each row. Dotted paths keep their shape ("data.date" → {data:{date}}); a trailing ":N" caps a string field ("description:130"), so the island carries what the card shows. Applied BEFORE the size limit.' },
        ],
        example:
          ...
          '{{sw-json-data pages.news._attributes.children id="posts" fields="title,path,image,description:130,data.date"}}\n' +
          '\n' +
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        description:
          'Set Website settings → Sticky header to fix the top navigation to the viewport so it stays visible as the page scrolls. The modes are POSITIONAL only — Pinned (always visible) or Hide on scroll down. There is no Shrink mode: how the bar LOOKS once scrolled (condense, colour, shadow) is always your own CSS keyed on html.sw-scrolled, because a condense has to know which row of YOUR header collapses. A fixed header is out of flow, so the platform pads the page for you AUTOMATICALLY — you do not need to add anything for content to clear the bar. Use sw-top-padding only to move that offset somewhere specific: on an inner element of a full-bleed hero/slider that should bleed UNDER the header (the background bleeds, the text clears). To change the AMOUNT of clearance use :root{--sw-header-offset:…} (0 = this page clears the bar itself) — never --sw-header-h, which is the bar HEIGHT and is also read by jump-link offsets and ScrollSpy. ⚠ sw-top-padding is silently defeated by competing padding on the SAME element: it is one class in the platform stylesheet and Tailwind utilities load after it, so p-4 / pt-* / py-*, a custom class that sets padding, or an inline style="padding…" wins and the class does nothing at all. Give that element side-specific padding instead (p-4 → px-4 pb-4). The offset reads the --sw-header-h token, a HARDCODED default sized for the stock header (4.5rem mobile / 4.75rem desktop), NOT measured from yours — so a custom header must set its real height in Custom CSS, at EVERY breakpoint the bar changes at, exactly: too small and headings sit behind the bar, too large and a strip of background paints just below it as a coloured edge under the nav. State hooks for your own scroll CSS: html.sw-scrolled (set once the page is scrolled — on every site, in every mode) and html.sw-nav-hidden (hide-on-scroll only — the bar is translated off-screen). The ENTRANCE animation is author-controlled — write it in Custom CSS (see the example).',
        example:
