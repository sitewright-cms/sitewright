...
vi.mock('../src/api', () => ({
  ...
  api: {
    getSettings: (o: string, p: string) => getSettings(o, p),
    putSettings: (o: string, p: string, b: SettingsBundle) => putSettings(o, p, b),
  },
}));
...
describe('SettingsView', () => {
  it('loads the identity and renders both section tabs', async () => {
    render(<SettingsView org={org} project={project} />);
  ...
  it('edits a field and saves the assembled bundle, then shows Saved', async () => {
    ...
    const sent = putSettings.mock.calls[0]![2] as SettingsBundle;
  ...
  it('switches to the Website section and edits siteUrl into the saved bundle', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Save changes' }));
    await waitFor(() => expect(putSettings).toHaveBeenCalledTimes(1));
    expect((putSettings.mock.calls[0]![2] as SettingsBundle).website?.siteUrl).toBe('https://acme.com');
  });
