...
| D5 | **AI is Claude-first behind an `AIProvider` interface** | Online (platform API) and offline (CLI) share one provider. The second adapter has since shipped: `provider: 'openai'` drives any OpenAI-compatible endpoint (hosted or self-hosted) via a configurable base URL, with no rearchitecting. |
