...
| `SW_SECURITY_CONTACT` | upstream advisory URL | Who fields a vulnerability report about **this instance**, published at `/.well-known/security.txt` ([RFC 9116](https://www.rfc-editor.org/info/rfc9116)). A comma-separated list of URIs in preference order — `https://…` (a form), `mailto:…`, or `tel:+…`; anything else is ignored. Setting it **replaces** the upstream default rather than adding to it. Unset → reports go to the project's private advisory channel, which is right for a stock instance and wrong once you patch your own build. |
...
| `SW_HEAPSNAPSHOT` | — (off) | Set to `1` to arm `--heapsnapshot-signal=SIGUSR2`. After the activity you want to inspect, `docker kill -s SIGUSR2 <container>` writes a `.heapsnapshot` next to the app; open it in Chrome DevTools → Memory. Use it to tell a real leak (reachable JS objects) from allocator retention (freed memory the allocator has not returned) — from outside they look identical. Off by default: writing one pauses the process and the file is roughly heap-sized. |
| `MALLOC_CONF` | `dirty_decay_ms:2000,muzzy_decay_ms:2000` | jemalloc tuning. Shortens how long freed pages are held before being returned to the OS (default 10s), so RSS comes back down sooner after a burst of image/render work. **Do not add `background_thread:true`**: it is the obvious way to make purging happen while fully idle, and it deadlocks forks — measured, the pagespeed audit then fails 3/3 with `could not launch a headless browser` because Chrome stalls between fork and exec. Set your own value to override entirely. |
