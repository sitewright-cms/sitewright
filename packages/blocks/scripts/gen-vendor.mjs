...
const TARGETS = [
  ...
  // Image Map — interactive hotspot maps. Unlike the targets above this bundles NO npm package:
  // the source in vendor-src/image-map/ is first-party (a licensed fork of Image Map Pro 6.1.11,
  // modified and integrated under an extended licence — see its README for what was removed and
  // renamed). So `libs` is empty, the MIT gate has nothing to check, and `notice` supplies the
  // banner instead. Its CSS comes from the BUNDLE (the entry imports the stylesheet tree) rather
  // than from a node_modules path, hence `css.fromBundle`.
  {
    entry: 'vendor-src/image-map.entry.js',
    out: 'src/vendor/image-map-runtime.ts',
    jsConst: 'IMAGE_MAP_RUNTIME_JS',
    libs: [],
    notice: 'Sitewright Image Map runtime. Contains code derived from Image Map Pro 6.1.11, used under licence.',
    css: { fromBundle: true, cssConst: 'IMAGE_MAP_VENDOR_CSS' },
    alias: {
      imap: 'vendor-src/image-map/src',
      'imap-shared': 'vendor-src/image-map/shared',
      'imap-css': 'vendor-src/image-map/assets',
    },
  },
];

function libBanner(libs, notice) {
  // A target with no npm dependencies carries its own notice — there is no package license to
  // gate or enumerate.
  if (libs.length === 0) return `/*! ${notice} */`;
...
for (const t of TARGETS) {
  const banner = libBanner(t.libs, t.notice);
  const result = await build({
    ...
    target: 'es2018',
    write: false,
    // A target whose entry imports CSS produces TWO output files; outdir just names them.
    outdir: join(pkgRoot, 'dist-vendor-tmp'),
    banner: { js: banner },
    legalComments: 'none',
  ...
  // Select outputs BY EXTENSION — with a CSS-importing entry the stylesheet can come first.
  const outFile = (ext) => result.outputFiles.find((f) => f.path.endsWith(ext));
  let js = outFile('.js').text.trimEnd();
  ...
  const parts = [
    ...
    t.libs.length > 0
      ...
      : `// Bundled from ${t.entry} (first-party source; no npm dependencies).`,
    '// Regenerate: pnpm --filter @sitewright/blocks gen:vendor',
    '',
    `export const ${t.jsConst}: string = ${JSON.stringify(js)};`,
  ];
  if (t.css) {
    // Either the stylesheet the ENTRY imported (bundled alongside the JS), or one/several files
    // read from node_modules and concatenated in order — e.g. a structural layout sheet plus a
    // single theme, to avoid shipping a vendor's other unused themes.
    let css;
    if (t.css.fromBundle) {
      css = (outFile('.css')?.text ?? '').trim();
      if (css === '') throw new Error(`gen-vendor: ${t.entry} declares css.fromBundle but emitted no stylesheet`);
    } else {
      const fromList = Array.isArray(t.css.from) ? t.css.from : [t.css.from];
      css = fromList.map((f) => readFileSync(join(pkgRoot, f), 'utf8').trim()).join('\n');
    }
