...
export const BANNER_CSS = [
  ...
  // below so an author's `data-sw-animation` can own the entrance instead of fighting it.
  ...
  // Built-in fade+rise reveal — applied ONLY when the author did NOT supply an animation effect (then
  ...
  // animation transform effect are free to animate the entrance without fighting the centering offset.
  ...
  '[data-sw-component="banner"][data-position="inline"]:not([data-sw-animation]){transform:none}',
...
//
// ENTRANCE: by default a banner fades+rises in. Add a `data-sw-animation` effect (fade-up /
// zoom-in / flip-left / …, plus data-sw-delay/-duration/-easing) and the banner yields the entrance
// to the shared animation runtime instead — the dismiss reverses whichever entrance was used.
export const BANNER_JS = `(function(){
  ...
  // Animation timing applied to the banner itself (the shared runtime skips banners): delay + duration
  // parseInt-clamped to [0,5000]ms; easing resolved through a fixed allowlist (a null-proto map, so a
  // hostile key can't reach an Object.prototype member) — values can never inject style.
  ...
  function applyAnim(root){
    var d=clampMs(root.getAttribute('data-sw-delay'));if(d)root.style.transitionDelay=d+'ms';
    var u=clampMs(root.getAttribute('data-sw-duration'));if(u)root.style.transitionDuration=u+'ms';
    var e=EASE[root.getAttribute('data-sw-easing')||''];if(e)root.style.transitionTimingFunction=e;
  }
  ...
  function reveal(root){
    var anim=root.getAttribute('data-sw-animation');
    var raf=window.requestAnimationFrame||function(cb){setTimeout(cb,16);};
    var run=function(){
      // An animated banner uses the effect classes for its ENTRANCE: apply 'sw-animation-init' (the
      // hidden state) BEFORE un-hiding so there is no flash, then drive 'sw-animation-active' on the next
      // frame. The reveal (not a scroll) is the trigger, so the banner owns these classes + the
      // data-sw-* timing itself. A plain banner uses the built-in fade via [data-sw-banner-shown].
      if(anim){applyAnim(root);root.classList.add('sw-animation-init');}
      root.removeAttribute('hidden');
      // Double-rAF for the animated path so the sw-animation-init state is guaranteed painted before
      // sw-animation-active, so the transition always runs; the plain path keeps its single-rAF reveal.
      if(anim)raf(function(){raf(function(){root.classList.add('sw-animation-active');});});
  ...
  function dismiss(root,key,mode){
    ...
    // EXIT mirrors the entrance: an animated banner reverses its effect (drop 'sw-animation-active' →
    // back to the 'sw-animation-init' transform/opacity); a plain banner drops the shown flag (fade/rise
    // out). Hide once the transition completes (the animation duration, default 400ms, vs the built-in .28s).
    var anim=root.getAttribute('data-sw-animation');
    var hideIn=300;
    if(anim){
      root.style.transitionDelay='0ms'; // clear the entrance stagger (sticky inline style) so the exit reversal starts NOW, not after data-sw-delay
      root.classList.remove('sw-animation-active');
      var d=clampMs(root.getAttribute('data-sw-duration'));
      hideIn=Math.min((d>0?d:400)+60,5100);
    }else{
  ...
  function enhance(root){
    ...
    if(root.getAttribute('data-sw-animation')==='')root.removeAttribute('data-sw-animation'); // empty effect → plain reveal (CSS :not([data-sw-animation]) must match)
