import { deepExtend, escapeHtml, isNumber } from 'imap-shared/utilities'
...
export default class Paragraph {
  ...
  html() {
    return `<div style="${this.css()} ${escapeHtml(this.options.other.css)}" id="${escapeHtml(
      this.options.other.id
    )}" class="${escapeHtml(this.options.other.classes)}">${this.options.text}</div>`
  }
}
