...
export interface ProjectBundle {
  ...
  forms?: readonly Form[];
  /** Interactive image maps (optional; consumed by `{{sw-imagemap}}` / the data-sw-imagemap pass). */
  imageMaps?: readonly ImageMap[];
}
...
export function validateProject(bundle: ProjectBundle): ValidationIssue[] {
  ...
  // Entry ids are DATASET-SCOPED: they only have to be unique WITHIN their owning dataset (two datasets
  // may each hold e.g. `intro` / `row_1`). Flag a collision only when the SAME id repeats in the SAME
  // dataset — key the duplicate check on the (dataset, id) pair, and report the id + its dataset.
  {
    const seen = new Set<string>();
    const reported = new Set<string>();
    for (const entry of bundle.entries) {
      const key = `${entry.dataset}\0${entry.id}`;
      if (seen.has(key) && !reported.has(key)) {
        issues.push({ code: 'duplicate_entry_id', message: `duplicate entry id: "${entry.id}" in dataset "${entry.dataset}"` });
        reported.add(key);
      }
      seen.add(key);
    }
