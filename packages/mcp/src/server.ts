...
/**
  ...
  server.registerTool(
    ...
    {
      description:
        'Create or REPLACE a page. The page id is taken from page.id. This is a TOTAL replace — every field you omit is deleted, so only use it when you are writing the whole page. TWO fields survive an omission: `parent` (a page always hangs off a home, so a replace never yanks a sub-page up to the root and moves its URL) and `data.swImport` (importer-owned provenance the fidelity tools need). To change a FEW fields (a nav label, the title, one data key) use patch_page instead. Returns a RECEIPT — { kind, id, bytes, created, changed } — not the page; call get_page if you need the stored page back. If the page CHANGED since you last read it (an operator editing in the browser), this is refused with a 409 rather than overwriting their work — re-read it with get_page, re-apply your change, and write again.',
