...
export function createSitewrightMcpServer(client: SitewrightClient, scope: Scope): McpServer {
  const server = new McpServer(
    ...
    // Admin-overridable instructions (set in the instance settings → agent panel), resolved by the
    // API into `scope.agentInstructions`; fall back to the built-in default when unset/older API.
    { instructions: scope.agentInstructions ?? DEFAULT_AGENT_INSTRUCTIONS },
  );
