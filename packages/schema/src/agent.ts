...
- HEADING TAGS belong to the PAGE's own source — that is the document outline, and where h1-h6 are exactly right. They are NOT used anywhere else: nothing the platform injects carries one (a tooltip headline, the cart drawer's title), and an h1-h6 inside RICH content (a \`data-sw-html\`/\`{{sw-html}}\` value, a dataset richtext field) is rewritten at the render sink to \`<p class="sw-hN">\` — pixel-identical, it just stops joining an outline whose levels the page's author never chose. For a headline LOOK inside rich content, reach for size + \`font-heading\` + bold (e.g. \`text-2xl font-heading font-bold\`), which is also all the rich-text toolbar offers.
...
- FROM YOUR OWN DISK, SMALL (<=256KB — an SVG logo, an icon, a favicon): upload_media with the bytes
  inline as base64 ({ filename, content_base64, folder? }). One call, no shell needed.
- FROM YOUR OWN DISK, LARGER: create_media_upload → a one-shot uploadUrl you send the file to yourself
  (curl -T ./photo.jpg "<uploadUrl>?filename=photo.jpg"). Two steps because past ~256KB the bytes must
...
WHEN A PAGE IS DONE (clone_audit returns pass ✓ AND visual_audit shows zero blocker+major): set
...
export const MCP_TOOL_CATALOG: readonly McpToolMeta[] = [
  ...
  { name: 'transform_image', description: "Rotate (90/180/270) and/or crop an image. IN PLACE by default — the id and URL are unchanged, so every reference keeps working; use it to CORRECT a photograph. Pass saveAs to write a new asset instead. Rotation applies before the crop.", capability: 'content:write' },
