...
export const BINDING_NAMESPACES: readonly BindingDoc[] = [
  ...
  },
  {
    ...
    description:
      'DIRECT access to ANOTHER page by slug PATH. Walk the page tree by slug from the home: pages.services is the top-level page slugged “services”, pages.services.seo its child slugged “seo”. Each node exposes .title, .slug, .path (its full route — use {{sw-url pages.x.path}}), .locale, .data (that page’s page.data), and .children (its direct child pages, the SAME array shape as page.children — so an overview on ANOTHER page can list a subtree, e.g. a home grid that lists the service pages). Same-locale: on a German page the slugs are the GERMAN ones (pages.leistungen.seo). An unknown path renders empty. NOTE: the node fields (data, title, path, slug, locale, children) take precedence, so a child page whose slug is exactly one of those words can’t be reached this way (rename it).',
    example:
