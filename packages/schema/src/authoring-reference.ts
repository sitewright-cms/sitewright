...
export const SW_HELPERS: readonly SwHelper[] = [
  ...
  { name: 'sw-icon', syntax: '{{sw-icon "name[:weight]" ["classes"]}}', summary: 'Inlines an SVG icon. A BARE name is a PHOSPHOR glyph, FILLED by default; ":weight" picks thin|light|regular|bold|fill|duotone (e.g. "check:thin"). A Lucide name still works (it maps to its Phosphor twin, else renders as a Lucide outline). "brand:slug" is a themed brand/social logo (e.g. "brand:whatsapp"). "x" ≠ "brand:x". Find names with search_icons.' },
  ...
  { name: 'sw-stagger', syntax: '{{sw-stagger @index [step] [max]}}', summary: 'The reveal DELAY in ms for item @index of a loop: index × step (default 100), capped at max (default 600) so a long list does not end up seconds behind. Use it inside {{#each}} to stagger a grid: data-sw-delay="{{sw-stagger @index 90}}". The engine has NO general arithmetic — this is the supported way to compute a per-item delay.' },
