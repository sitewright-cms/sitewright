...
export const SW_HELPERS: readonly SwHelper[] = [
  ...
  { name: 'sw-json-data', syntax: '{{sw-json-data value id="products" [type="application/ld+json"]}}', summary: 'HANDS DATA TO A SCRIPT: emits the value as an inert island — <script type="application/json" id="…">…</script> — read with JSON.parse(document.getElementById("…").textContent). The ONLY way to do this: a template may not interpolate inside a <script> body at all (a value there could close the tag), so the helper emits the whole element and escapes the payload. Use for a list the page filters/sorts/paginates client-side, a widget config, or your own ld+json. REFUSES with a visible HTML comment rather than emitting something wrong: a whole ambient namespace (pass a projection like dataset.products), a credential-shaped key, an unserializable value, or anything over 256 KB — for a list that big declare a website.dataFiles entry and fetch it instead.' },
