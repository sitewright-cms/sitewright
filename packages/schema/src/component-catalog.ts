...
export const COMPONENT_CATALOG: readonly ComponentCatalogEntry[] = [
  {
    ...
    attributes: [
      ...
      { name: 'data-align', on: 'root', description: 'Fallback snap alignment: "start" (default), "center", or "end". Prefer data-item-align (it covers both the scrolling snap AND the underfull-row case).' },
      { name: 'data-item-align', on: 'root', description: 'Horizontal alignment: "start" (default), "center", or "end". Drives BOTH the Embla snap alignment (so "center" centres the ACTIVE slide with a peek on each side; with loop off, containScroll clamps the first slide left + last right) AND justify-content for an UNDERFULL row that does not scroll (fewer slides than --sw-items). The one alignment knob — use it for multi-item/peek sliders.' },
