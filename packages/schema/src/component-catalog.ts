...
export const COMPONENT_CATALOG: readonly ComponentCatalogEntry[] = [
  ...
  <div data-sw-part="panel" data-sw-title="Second tab">Second panel content</div>
    ...
    notes:
      "Use this whenever tabs switch CONTENT. The markup is minimal: a `data-sw-component=\"tabs\"` root with a `data-sw-part=\"panel\"` per tab — the runtime generates the tablist (creating the mount if you omit it), the tab buttons, the ARIA roles, and roving tabindex (no `data-sw-block` or `role=` attributes to author). Tab labels are BOLD; the active tab shows white text over a floating primary selector pill that glides to it; inactive tabs use the default text colour and turn primary on hover; the buttons get a press ripple; and each panel fades in (re-triggers on every switch). For a PLAIN label use `data-sw-title=\"…\"` on the panel. For an HTML label (icon, bold, badge) add a `data-sw-part=\"tabtitle\"` element as a DIRECT child of the panel — the runtime moves its (already server-rendered) nodes into the button, so this is XSS-safe in a way `data-sw-title` HTML never could be (the attribute resolves from content-editor-editable page.data and is therefore always escaped to text); keep `data-sw-title` for the accessible name. Put only NON-interactive markup in a tabtitle (no `<a>`/`<button>`/`<input>` — the nodes become children of the generated `<button>`). Author a `data-sw-part=\"tablist\"` element yourself only to style the tab bar (e.g. `class=\"px-2\"`) — the runtime still fills it. DaisyUI's `tabs`/`tab` classes are for tab-STYLED navigation links (a row of links that navigate); do not build DaisyUI radio-input content tabs — they lack tablist semantics and their adjacency-dependent markup is brittle.",
    examples: [
      {
        label: 'Rich tab labels (icon + text)',
        code: '<div data-sw-component="tabs">\n  <div data-sw-part="panel" data-sw-title="Overview">\n    <span data-sw-part="tabtitle">{{sw-icon "sparkles" "size-4"}} Overview</span>\n    Panel one content\n  </div>\n  <div data-sw-part="panel" data-sw-title="Pricing">\n    <span data-sw-part="tabtitle">{{sw-icon "tag" "size-4"}} <b>Pricing</b></span>\n    Panel two content\n  </div>\n</div>',
        note: 'The tabtitle nodes are moved into the button (HTML labels, no XSS sink). data-sw-title stays as the aria-label.',
      },
    ],
  },
  {
