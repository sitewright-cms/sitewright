...
export function configurePageNav(pages: Page[], siteName?: string): void {
  ...
  // TOP-LEVEL = home + the pages that render at `/<slug>`: home's direct children (the importer parents
  // top-level pages to home) OR a parentless root sibling (the older structure) — handle both so the header
  // menu is the same either way. Their OWN children become dropdown items.
  ...
  for (const p of topLevel) {
    ...
    p.nav = {
      slots: ['header'],
      order: o,
      // A dropdown when this top-level page has children of its OWN (home does not — its "children" ARE the
      // top-level pages, which are the menu itself, not a dropdown under it).
      ...(!isHome(p) && childParentIds.has(p.id) ? { dropdown: true } : {}),
      title: isHome(p) ? 'Home' : cleanNavLabel(p.title, siteName),
    };
  }
  // Deeper children (NOT a top-level nav item): order within their sibling group; remove any (empty-slots)
  // nav object so the PUT validates and they nest under the dropdown parent rather than appearing flat.
